...
*Ketik pertanyaan Anda di atas, dan saya akan meramu jawaban komprehensif berdasarkan data riil keuangan Anda!*`;
  ...
  // ==================== DEVELOPER & SYSTEM ADMIN ENDPOINTS ====================
  // 1. GET Current System status & metrics
  app.get('/api/dev/status', (req, res) => {
    ...
    res.json({
      ...
      memory: {
        rss: Math.round(memory.rss / 1024 / 1024), // in MB
  ...
  // 3. POST Save Environment Variables
  app.post('/api/dev/env', (req, res) => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(__dirname, '.env'), fileContent, 'utf8');
  ...
  // 4. POST Force simulated cluster failover (Worker Crash)
  app.post('/api/dev/crash', (req, res) => {
