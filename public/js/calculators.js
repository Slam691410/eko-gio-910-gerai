...
function calculateKHL() {
  ...
  // Render 64-Component Permenaker No. 18/2020 KHL list
  renderPermenakerKHLBreakdown(totalPeople, baseKhlPerPerson);
}

function renderPermenakerKHLBreakdown(totalPeople, baseKhl) {
  const container = document.getElementById('khl-permenaker-breakdown-container');
  if (!container) return;

  const khlGroups = [
    { name: "1. Makanan & Minuman (13 Jenis)", weight: 0.35, items: ["Beras Kualitas Sedang (10kg)", "Daging Sapi/Ayam Berkualitas", "Ikan Segar Berkualitas", "Telur Ayam Ras", "Tahu & Tempe Higienis", "Susu Bubuk Berkualitas", "Gula Pasir", "Minyak Goreng Curah", "Sayur-sayuran Hijau", "Buah-buahan Lokal", "Teh & Kopi Bubuk", "Bumbu-bumbuan & Garam", "Air Minum Bersih"] },
    { name: "2. Sandang & Pakaian (14 Jenis)", weight: 0.15, items: ["Celana Panjang/Rok Kerja", "Kemeja Lengan Pendek/Panjang", "Kaos Oblong", "Sepatu Kerja Kulit/Karet", "Sandal Rumah & Santai", "Kaos Kaki Katun", "Pakaian Dalam Berkualitas", "Sabun Cuci Pakaian", "Jasa Potong Rambut Bulanan", "Perlengkapan Ibadah (Sajadah/Mukena)", "Seterika Listrik (Amortisasi)", "Tas Kerja/Kulit", "Payung / Jas Hujan", "Semir & Sikat Sepatu"] },
    { name: "3. Perumahan & Papan (22 Jenis)", weight: 0.28, items: ["Sewa Kamar / Rumah Sederhana", "Tarif Listrik Bulanan (900VA)", "Tarif Air PDAM Bersih", "Sabun Mandi Keluarga", "Kasur & Bantal Busa", "Sprei & Sarung Bantal", "Lemari Pakaian Kayu", "Kompor Gas & Selang Regulator", "Tabung Gas LPG 3kg / 12kg", "Alat Makan (Piring, Gelas)", "Sendok & Garpu Stainless", "Meja & Kursi Makan Kayu", "Sapu & Alat Pel Lantai", "Ember & Gayung Plastik", "Televisi Berwarna (Amortisasi)", "Kipas Angin Listrik", "Gorden Jendela Rumah", "Keset Kaki Serabut", "Wadah Sampah Tertutup", "Cermin Dinding Kaca", "Lampu LED Hemat Energi", "Jasa Kebersihan Lingkungan"] },
    { name: "4. Pendidikan & Telekomunikasi (2 Jenis)", weight: 0.08, items: ["Bahan Bacaan / Buku / Koran E-Paper", "Kuota Pulsa & Paket Data Internet"] },
    { name: "5. Kesehatan & Sanitasi (5 Jenis)", weight: 0.05, items: ["Sarana Klinik / Obat-obatan Bebas", "Pasta Gigi Ukuran Besar", "Sikat Gigi Karet", "Pembalut Wanita / Perlengkapan Sanitasi", "Cairan Antiseptik Pembersih"] },
    { name: "6. Transportasi Publik (1 Jenis)", weight: 0.04, items: ["Biaya Transportasi Umum / KRL / MRT / Bensin"] },
    { name: "7. Rekreasi, Tabungan & Lainnya (7 Jenis)", weight: 0.05, items: ["Rekreasi Lokal / Bioskop / Taman", "Tabungan Otonom Masa Depan", "Keperluan Sosial / Zakat Maal", "Jasa Pos / Logistik", "Keperluan Tak Terduga", "Perawatan Pribadi (Skin Care Dasar)", "Alat Cukur / Pisau Cukur"] }
  ];

  container.innerHTML = '';
  const totalKhlVal = totalPeople * baseKhl;

  khlGroups.forEach((group, groupIdx) => {
    const groupSum = totalKhlVal * group.weight;
    const itemCost = groupSum / group.items.length;

    const groupDiv = document.createElement('div');
    groupDiv.className = "bg-slate-900 border border-cyber-border/40 rounded-xl overflow-hidden mb-2.5";
    
    groupDiv.innerHTML = `
      <button onclick="toggleKhlGroup(${groupIdx})" class="w-full p-3 flex justify-between items-center text-xs font-bold text-slate-200 hover:bg-slate-850 transition duration-150 outline-none">
        <span class="flex items-center gap-1.5 text-slate-200">
          <i data-lucide="chevron-right" class="w-3.5 h-3.5 text-brand-500 transition-transform duration-200" id="khl-chevron-${groupIdx}"></i>
          ${group.name}
        </span>
        <strong class="font-mono text-brand-400">Rp ${Math.round(groupSum).toLocaleString('id-ID')} / bln</strong>
      </button>
      <div id="khl-group-content-${groupIdx}" class="hidden p-3 border-t border-cyber-border/20 bg-slate-950/60 space-y-1.5">
        ${group.items.map((item, idx) => `
          <div class="flex justify-between items-center text-[10px] text-slate-400 font-mono">
            <span>${idx + 1}. ${item}</span>
            <span class="text-slate-500">Rp ${Math.round(itemCost).toLocaleString('id-ID')}</span>
          </div>
        `).join('')}
      </div>
    `;
...
function toggleKhlGroup(index) {
  const content = document.getElementById(`khl-group-content-${index}`);
  const chev = document.getElementById(`khl-chevron-${index}`);
  if (content && chev) {
...
// -------------------------------------------------------------
// DYNAMIC DEBT SNOWBALL PAYOFF AMORTIZATION SIMULATOR

function simulateDebtSnowballPayoff() {
  if (!appState.db || !appState.db.debts || appState.db.debts.length === 0) {
    alert('Belum ada data utang rill yang terdaftar.');
    return;
  }

  const container = document.getElementById('debt-snowball-result-container');
  if (!container) return;

  container.innerHTML = '';
  container.classList.remove('hidden');

  const extraPayInput = document.getElementById('debt-snowball-extra-payment');
  const extraPayment = parseFloat(extraPayInput ? extraPayInput.value : 1000000) || 0;

  // Clone debts so we don't modify the database
  const debts = appState.db.debts.map(d => ({
    name: d.name,
  ...
  let month = 1;
  let hasActiveDebts = true;
  const logLines = [];

  logLines.push(`[SYSTEM] MEMULAI SIMULASI PELUNASAN DEBT SNOWBALL`);
  logLines.push(`[SYSTEM] Dana Tambahan Bulanan dialokasikan: Rp ${extraPayment.toLocaleString('id-ID')} IDR.`);
  logLines.push(`[SYSTEM] Urutan Pelunasan (Saldo Terkecil): ${debts.map(d => d.name).join(' ➜ ')}`);
  logLines.push(`--------------------------------------------------`);

  while (hasActiveDebts && month <= 120) { 
    let currentExtra = extraPayment;
    let paidThisMonth = 0;
    
    logLines.push(`➜ **BULAN ke-${month}**`);
    ...
    debts.forEach(debt => {
      if (debt.remaining > 0) {
        ...
        const actualPay = Math.min(debt.remaining, debt.minPayment);
        debt.remaining -= actualPay;
        paidThisMonth += actualPay;
      }
    });
    ...
    for (let i = 0; i < debts.length; i++) {
      const debt = debts[i];
      if (debt.remaining > 0) {
        const extraPay = Math.min(debt.remaining, currentExtra);
        debt.remaining -= extraPay;
        currentExtra -= extraPay;
        paidThisMonth += extraPay;
        
        if (debt.remaining === 0) {
          logLines.push(`   🎉 **LUNAS!** Kewajiban [${debt.name}] berhasil dilunasi sepenuhnya!`);
    ...
    debts.forEach(debt => {
      if (debt.remaining > 0) {
        logLines.push(`   • ${debt.name}: Sisa Saldo Rp ${Math.round(debt.remaining).toLocaleString('id-ID')}`);
      }
    });

    if (activeCount === 0) {
      logLines.push(`--------------------------------------------------`);
      logLines.push(`🎉 **SUCCESS!** Seluruh kewajiban utang Anda berhasil dilunasi dalam **${month} Bulan**!`);
  ...
  if (month > 120) {
    logLines.push(`--------------------------------------------------`);
    logLines.push(`⚠️ Simulasi dihentikan karena melampaui masa 10 tahun (120 bulan). Harap naikkan alokasi dana tambahan.`);
  }

  logLines.forEach(line => {
    const div = document.createElement('div');
    div.className = "pb-1 mb-1 border-b border-cyber-border/10";
    if (line.includes('LUNAS') || line.includes('SUCCESS')) {
      div.className += " text-emerald-400 font-bold";
    } else if (line.includes('BULAN')) {
