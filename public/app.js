...
// ==================== MAIN FULLSTACK CUSTOMER GROW-HACK & WEB3 WARIS CONSOLE ====================

let mainHeartbeatDays = 365;

// 1. Simulates 5 new user checkouts for Eko Gio's referral
async function executeSimulatedReferralLoop() {
  if (!appState.db) return;

  const referralBonusTotal = 495000; // Rp 99.000 * 5 * 100% equivalent referral earnings split or custom
  const paxgEquivalent = 0.005; // 0.005 PAXG gold

  appState.db.affiliateData.clicks = (appState.db.affiliateData.clicks || 0) + 12; // Simulate some clicks too
...
function triggerMainOwnerHeartbeat() {
  mainHeartbeatDays = 365;
  updateMainHeartbeatUI();
...
function triggerMainSimulateTimePass() {
  mainHeartbeatDays = 0;
  updateMainHeartbeatUI();
...
function renderMainBlockchainHeirs() {
  ...
  heirsList.forEach(heir => {
    const div = document.createElement('div');
    div.className = "flex justify-between items-center bg-slate-900 p-2 rounded.5 border border-cyber-border/20 text-[11px] font-mono";
    
    let actionBtn = "";
    if (mainHeartbeatDays <= 0) {
      actionBtn = `<button onclick="executeMainInheritanceClaim('${heir.name}', '${heir.share}')" class="bg-red-950 hover:bg-red-800 text-red-400 border border-red-800/40 px-2 py-0.5 rounded text-[9px] font-bold uppercase transition">Klaim Waris</button>`;
    } else {
      actionBtn = `<span class="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Locked</span>`;
    }
...
// 6. Executes Simulated On-Chain Inheritance Claim on Main App
function executeMainInheritanceClaim(heirName, share) {
  // Read and calculate platform accumulated fees
  let totalPlatformFees = 0;
  if (appState.db && appState.db.affiliateData && appState.db.affiliateData.history) {
    appState.db.affiliateData.history.forEach(item => {
      const pShare = item.platformShare || Math.round(item.amount * (30/70));
  ...
  setTimeout(() => {
    ...
    // Decrease holding balance for visual feedback
    mainHeartbeatDays = 365; // Reset UI
    updateMainHeartbeatUI();
  }, 1200);
}
